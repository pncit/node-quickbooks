...
- Bump three dependencies in `package.json`: `underscore` → `^1.13.8`, `fast-xml-parser` → `^5.9.2`, `uuid` → `^14.0.0`.
...
|------|-------------|-----------|
| `package.json` | Dependency version edits (3 lines) | Apply the three security bumps per plan Step 1 |
...
- `"uuid": "^8.3.2"` → `"uuid": "^14.0.0"` — clears moderate advisory GHSA-w5hq-g745-h8pq (fixed `>=11.1.1`); caret-pinned to the verified major (`uuid.v1()` tested; only one call site at `index.js:2352`). All other dependencies left at upstream's pins.
...
- Belt-and-suspenders: none of `request`, `request-debug`, `form-data`, `tough-cookie`, `qs`, `uuid`, `underscore`, `fast-xml-parser` appear as vulnerability keys in the audit JSON.
...
- `node -e "var u=require('uuid'); if(typeof u.v1!=='function'||!u.v1())process.exit(1)"` exits 0; `uuid.v1()` returns a valid v1 UUID under `uuid@14.0.0`.
...
- **uuid major bump is safe for the one call site:** `uuid.v1()` is not the vulnerable API path (that was `v3`/`v5`/`v6`), and the smoke check confirms CommonJS `require('uuid').v1` resolves and returns a valid UUID under `14.0.0`.
- **No new dependencies added:** zero new direct or devDependencies introduced. The changes are strictly version bumps of existing direct dependencies.
...
grep -qF '"uuid": "^14.0.0"' package.json              # OK
```
